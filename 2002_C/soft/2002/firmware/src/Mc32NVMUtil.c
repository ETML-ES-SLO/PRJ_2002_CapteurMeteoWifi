...
#include "C:\microchip\harmony\v2_06\framework\peripheral\nvm\plib_nvm.h"
#include <sys/kmem.h>


// Row dans flash pour data
const uint32_t  eedata_addr[DEVICE_ROW_SIZE_DIVIDED_BY_4 ] __attribute__((aligned(1024), space(prog)));
...
void NVM_ResetAndWriteRow(uint32_t destAddr, uint32_t srcAddr) {
    // Désactiver les interruptions
    __builtin_disable_interrupts();

    // Réinitialisation des registres NVM
    PLIB_NVM_MemoryModifyInhibit(NVM_ID_0);
    PLIB_NVM_FlashWriteKeySequence(NVM_ID_0, 0x0);
    ...
    // Réactiver la modification mémoire
    PLIB_NVM_MemoryModifyEnable(NVM_ID_0);

    // Alignement sur la destination et la source
    PLIB_NVM_FlashAddressToModify(NVM_ID_0, virtualToPhysical(destAddr));
    PLIB_NVM_DataBlockSourceAddress(NVM_ID_0, virtualToPhysical(srcAddr));
    ...
    // Désactiver les modifications pour sécuriser l'opération
    PLIB_NVM_MemoryModifyInhibit(NVM_ID_0);

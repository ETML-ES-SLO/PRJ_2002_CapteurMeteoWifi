...
//    if(Struct_save.ESP8266_save.Magic == 0x12345678)
//    {
...
//    else
//    {
//        //sinon, on met une valeur par défaut (600 [s] = 10min)
//        ESP8266.Interval = 600;
//    }
    
    //test après la lecture dans la flash, si on retrouve la valeur du Magic
    if(Struct_save.Wifi_save.Magic == 0x12345678)
    {
        ...
        memmove(Wifi.DomoIp, Struct_save.Wifi_save.DomoIp, sizeof(Struct_save.Wifi_save.DomoIp));
        memmove(Wifi.DomoPort, Struct_save.Wifi_save.DomoPort, sizeof(Struct_save.Wifi_save.DomoPort));
    }
    else
    {
        ...
        memmove(Wifi.DomoIp, "192.168.0.104", strlen("192.168.0.104"));
        memmove(Wifi.DomoPort, "8080", strlen("8080"));
    }
    ...
    if(ESP8266_isStarted() == OK)
    {
        ...
        {
            //SCA todo ici voir si code de ce else sensé
            ESP8266_ChangeMode(ModeStation);
            ESP8266_GetIpAndMac(ESP8266.IP, ESP8266.MAC);
...
/*******************************************************************************
* Auteur : Diego Rickenbach                                                    *                     
* Nom de la fonction :     ESP8266_ChangeMode                                        *                                    
* Entrée :      uint8_t mode                                                   *                                                                      
* Sortie :      OK :        Status = 0                                         *
*               ERROR :     Status = 1                                         *
*               NO_DATA :   Status = 2                                         *
...
* Remarques:    Voir le prototype dans ESP8266.h.                              *
*******************************************************************************/
uint8_t ESP8266_ChangeMode(uint8_t mode)
{
...
/*******************************************************************************
* Auteur : Diego Rickenbach                                                    *                     
* Nom de la fonction :     ESP8266_GetIpAndMac                                          *                                    
...
uint8_t ESP8266_GetIpAndMac(char* StrIP, char* StrMAC)
{
    ...
    ReadIpAndMac((char*)StrRes, StrIP, StrMAC);
...
{  
    ...
    int8_t StrRes[500] = {0};
    uint8_t Status;
    //char IPDom[15]= "192.168.0.104";
    //char PortDom[4]="8080";
    
    sprintf((char*)httpString, "GET /json.htm?type=command&param=udevice&idx=%s&nvalue=0&svalue=%.1f;%.1f;%d;%.1f;%d HTTP/1.0\r\n\r\n", 
            Wifi.DomoIdx, (double)Temp, (double)Hum , Hum_stat, (double)Press, Press_fore);        
    httpStringLen = strlen((char*)httpString);
    
    sprintf ((char*)cmdString, "AT+CIPSEND=%2d\r\n", httpStringLen);
    cmdStringLen = strlen((char*)cmdString);    
    
    if(ESP8266_ConnectToTCPServer(Wifi.DomoIp,Wifi.DomoPort) == OK)
    {
